...
class HtoBBGG_ggH(HggBaseProcessor):
    ...
    def process(self, events: awkward.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            bquarks = bquarks[awkward.argsort(bquarks.pt, ascending=False)]
            bquarks["charge"] = awkward.zeros_like(
                bquarks.pt
            )  # added this because charge is not a property of photons in nanoAOD v11. We just assume every photon has charge zero...
            dibquark = awkward.combinations(
                bquarks, 2, fields=["bquark_lead", "bquark_sublead"]
            )
...
#------------------------------------store bbgg objects info--------------------------------------------------------------------------
        ...
        if self.output_location is not None:
            if self.output_format == "root":
                df_bquarks = diphoton_list_to_pandas(self, dibquark)  # same converter works fine
            else:
                akarr_bquarks = diphoton_ak_array(self, dibquark)
                akarr_bquarks = akarr_bquarks[[f for f in akarr_bquarks.fields if "lead_fixedGridRhoAll" not in f]]
            ...
            subdirs_bquarks = []
            if "dataset" in events.metadata:
                subdirs_bquarks.append(events.metadata["dataset"])
            subdirs_bquarks.append(do_variation)
            subdirs_bquarks.append("dibquark")   # store dijet files here

            if self.output_format == "root":
                dump_pandas(self, df_bquarks, fname_bquarks, self.output_location, subdirs_bquarks)
            else:
                dump_ak_array(self, akarr_bquarks, fname_bquarks, self.output_location, metadata, subdirs_bquarks)
